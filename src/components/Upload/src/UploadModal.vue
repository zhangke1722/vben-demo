<template>
  <BasicModal width="800px" :title="'上传'" :okText="'保存'" v-bind="$attrs" @register="register" @ok="handleOk"
    :closeFunc="handleCloseFunc" :maskClosable="false" :keyboard="false" class="upload-modal"
    :okButtonProps="getOkButtonProps" :cancelButtonProps="{ disabled: isUploadingRef }">
  ...
  setup(props, { emit }) {
    ...
    function handleOk() {
      ...
      if (fileList.length <= 0) {
        return createMessage.warning('无法上传成功的文件,无法保存!');
      }
      fileListRef.value = [];
      closeModal();
